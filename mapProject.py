...
def color(elev):
    ...
    if elev in range(minimun, minimun+step):
        col='green'
    elif elev in range(minimun+step, minimun+step+step):
        col='orange'
    else:
        col='red'
    return col
...
for lat, lon, name, elev in zip(df['LAT'], df['LON'], df['NAME'], df['ELEV']):
    ...
    fg.add_child(folium.Marker(location=[lat, lon], popup=name, icon=folium.Icon(color=color(elev), icon_color='gray')))
map.add_child(fg)
